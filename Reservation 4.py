...
class YouTubeExtractor:
    def __init__(self):
        ...
        self._ytdl = None
        self._cache = {}
        self._cache_timeout = 600  # Increased to 10 minutes
    ...
    async def extract_info(self, query: str, extract_flat: bool = False, retries: int = 3) -> Optional[Dict]:
        """Extract video/playlist information from YouTube with caching and retry logic"""
        ...
        for attempt in range(retries):
            try:
                ...
                # Clean old cache entries and limit size
                ...
                last_error = "timeout"
                logger.warning(f"YouTube extraction timeout (attempt {attempt + 1}/{retries}): {query}")
                if attempt < retries - 1:
                    await asyncio.sleep(1 * (attempt + 1))  # Exponential backoff

            except Exception as e:
                last_error = str(e)
                logger.warning(f"YouTube extraction failed (attempt {attempt + 1}/{retries}): {e}")
                if attempt < retries - 1:
                    await asyncio.sleep(1 * (attempt + 1))  # Exponential backoff

        logger.error(f"YouTube extraction failed after {retries} attempts for {query}: {last_error}")
        return None

    async def search_youtube(self, query: str, max_results: int = 10) -> List[Track]:
        """Search YouTube and return Track objects with better error handling"""
        ...
        try:
            ...
            if not info:
                logger.warning(f"No info returned from YouTube for: {query}")
                return []

            if "entries" not in info:
                logger.warning(f"No entries in YouTube response for: {query}")
                return []
            ...
            for entry in info["entries"]:
                ...
                try:
                    ...
                    if duration and duration > MAX_TRACK_LENGTH:
                        logger.debug(f"Skipping track (too long): {entry.get('title', 'Unknown')}")
                        continue
                    ...
                    if not title or not url:
                        logger.warning(f"Skipping track with missing title or URL")
                        continue
                    ...
                    logger.warning(f"Failed to create track from entry: {e}")
                    continue

            logger.info(f"✅ Found {len(tracks)} valid track(s) for: {query}")
            return tracks
    ...
    async def get_playlist_tracks(self, url: str, max_tracks: int = 50) -> List[Track]:
        """Extract all tracks from YouTube playlist"""
        try:
            logger.info(f"Extracting YouTube playlist: {url}")

            # First extract with flat mode to get all video URLs quickly
            opts = self.ytdl_opts.copy()
            ...
            tracks = []
            entries = result["entries"][:max_tracks]  # Limit playlist size

            logger.info(f"Found {len(entries)} videos in playlist, processing...")
            ...
            for entry in entries:
                ...
                try:
                    # Extract full info for each track
                    ...
                    if duration and duration > MAX_TRACK_LENGTH:
                        logger.debug(f"Skipping track (too long): {track_info.get('title')}")
                        continue
                    ...
                    logger.warning(f"Failed to extract track from playlist: {e}")
                    continue

            logger.info(f"✅ Successfully extracted {len(tracks)} tracks from YouTube playlist")
            return tracks
