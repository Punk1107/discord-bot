# -*- coding: utf-8 -*-
"""
🎵 Enhanced YouTube Music Bot v3.0 - Ultimate Edition
Production-ready Discord music bot with enterprise-level stability

🚀 Features:
- YouTube-only focus with advanced search
...
class YouTubeExtractor:
    ...
    def _get_ytdl(self):
        """Get yt-dlp instance with lazy initialization"""
        if self._ytdl is None:
            self._ytdl = yt_dlp.YoutubeDL(self.ytdl_opts)
        return self._ytdl
    
    def _clean_url(self, url: str) -> str:
        """Clean YouTube URL from playlist parameters"""
        try:
    ...
    async def extract_info(self, query: str, extract_flat: bool = False) -> Optional[Dict]:
        """Extract video/playlist information from YouTube with caching"""
        try:
            # Clean URL
            ...
            logger.warning(f"YouTube extraction timeout for {query}")
            return None
        except Exception as e:
            logger.error(f"YouTube extraction failed for {query}: {e}")
            return None
    
    async def search_youtube(self, query: str, max_results: int = 10) -> List[Track]:
        """Search YouTube and return Track objects"""
        try:
    ...
    async def get_track_from_url(self, url: str) -> Optional[Track]:
        ...
        try:
            ...
            # Handle playlist - get first video
...
class AudioEffectsProcessor:
    ...
    def build_ffmpeg_args(self, effects: List[AudioEffect], volume: float = 1.0, 
                         start_time: int = 0, quality: AudioQuality = AudioQuality.MEDIUM) -> Dict[str, str]:
        ...
        # Add normalization for better audio quality
        filters.append("dynaudnorm=f=75:g=25:p=0.55")
        
        # Quality settings
        bitrate = quality.value[0]
...
class EnhancedMusicBot(commands.Bot):
    def __init__(self):
        ...
        self.volume: Dict[int, float] = defaultdict(lambda: 0.7)
            ...
            try:
                ...
                for guild_id, config_data in rows:
                    try:
                        ...
                        self.server_configs[guild_id] = ServerConfig(**config_dict)
                    except Exception as e:
    ...
    async def save_server_config(self, config: ServerConfig):
        ...
        try:
            conn = await self.db.get_connection()
            try:
                config_data = json.dumps(asdict(config))
                await conn.execute(
    ...
    async def add_to_queue(self, guild_id: int, channel_id: int, track: Track):
        ...
        try:
            ...
            # Save to database with new connection
            conn = await self.db.get_connection()
            try:
                await conn.execute(
    ...
    async def remove_from_queue(self, guild_id: int, position: int) -> Optional[Track]:
        ...
        try:
            ...
            if 1 <= position <= len(queue):
                ...
                # Update database with new connection
                conn = await self.db.get_connection()
                try:
    ...
    async def play_next(self, guild_id: int):
        ...
        try:
            ...
            # Extract audio URL with retry mechanism
            ...
            volume = self.volume.get(guild_id, 0.7)
    ...
    async def send_now_playing(self, guild_id: int, track: Track):
        ...
        try:
            ...
            # Verify track is actually the currently playing track
            current_track = self.now_playing.get(guild_id)
            ...
            # Use last interaction channel or find suitable channel
            ...
            effects = self.effects.get(guild_id, [])
            if effects:
                effect_names = [effect.value for effect in effects[:3]]  # Show first 3
            ...
            embed.add_field(
                name="🔊 Settings",
                value=f"Volume: {int(self.volume.get(guild_id, 0.7) * 100)}% • Quality: {config.quality.value[1]}",
                inline=True
            )
    ...
    async def update_progress(self, guild_id: int, message: discord.Message, track: Track):
        ...
        try:
            while True:
                ...
                # Verify track is still the current track
                current_track = self.now_playing.get(guild_id)
                ...
                try:
                    ...
                    for i, field in enumerate(embed.fields):
                        ...
                        # Add progress field if not found
    ...
    async def cleanup_task(self):
        ...
        try:
            ...
            # Clean old history with new connection each time
    ...
    async def stats_updater(self):
        ...
        try:
            ...
            # Record metrics to database with new connection
            conn = await self.db.get_connection()
            try:
                metrics = [
...
class EffectsDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Note: Effects will apply to next track to avoid interrupting current playback
...
class SearchResultView(discord.ui.View):
    def __init__(self, bot: EnhancedMusicBot, tracks: List[Track], user_id: int):
        ...
        options = []
        for i, track in enumerate(tracks[:10], 1):  # Max 10 options
...
@app_commands.command(name="play", description="🎵 Play music from YouTube (URL or search)")
...
async def play_command(interaction: discord.Interaction, query: str):
    ...
    try:
        ...
        if bot.youtube.is_youtube_url(query):
...
async def queue_command(interaction: discord.Interaction, page: int = 1):
    ...
    embed.add_field(
        name="🔊 Volume",
        value=f"{int(bot.volume.get(guild_id, 0.7) * 100)}%",
        inline=True
    )
...
async def windto_command(interaction: discord.Interaction, time: str):
    ...
    try:
        ...
        vc = interaction.guild.voice_client
        vc.stop()  # This will trigger play_next with the new seek position
...
async def nowplaying_command(interaction: discord.Interaction):
    ...
    embed.add_field(name="🔊 Volume", value=f"{int(bot.volume.get(guild_id, 0.7) * 100)}%", inline=True)
...
async def history_command(interaction: discord.Interaction, limit: int = 10):
    ...
    try:
        async with await bot.db.get_connection() as conn:
            # Get recent tracks
...
async def stats_command(interaction: discord.Interaction):
    ...
    try:
        ...
        volume = int(bot.volume.get(interaction.guild.id, 0.7) * 100)
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.set_footer(text="🎵 Enhanced Music Bot v3.0 • Built for stability and performance")
    await interaction.followup.send(embed=embed)
...
async def on_voice_state_update(member, before, after):
    ...
    # Check all voice clients for idle status
